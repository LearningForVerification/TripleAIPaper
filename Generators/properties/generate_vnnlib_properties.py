...
def generate_local_robustness_property(
    ...
    """Generate a VNNLIB local robustness property (negated form)."""
    if noise_level < 0:
    ...
    with open(property_path, "w") as f:
        ...
        # Negated robustness property
        f.write("\n; Negation of local robustness (existence of a counterexample)\n")
        f.write("(assert (or\n")
        for i in range(num_classes):
            if i != correct_label:
                f.write(f"    (>= Y_{i} Y_{correct_label})\n")
        f.write("))\n")
