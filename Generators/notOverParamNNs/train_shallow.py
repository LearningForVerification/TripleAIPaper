...
def save_models(results: Dict[int, Dict], save_dir: str = 'models'):
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Hidden Neurons', 'Train Acc', 'Test Acc', 'Train Loss', 'Test Loss', 'Best Epoch', 'Total Epochs'])
        for hidden_dim, result in results.items():
            ...
            # Salvataggio modello ONNX
            onnx_path = os.path.join(save_dir, f'fcnn_{hidden_dim}.onnx')
            result['model'].eval()
            dummy_input = torch.randn(1, 1, 28, 28, device=next(result['model'].parameters()).device)
            torch.onnx.export(
                result['model'],
                dummy_input,
                onnx_path,
