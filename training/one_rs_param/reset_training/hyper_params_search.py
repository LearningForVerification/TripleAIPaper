...
from training.utils.nn_models import FCHeadOnly



def _generate_model(model_cls, basic_block, candidates_network_archs):
    to_ret_list = list()
    for hidden_dim in candidates_network_archs:
        model = model_cls(basic_block, [2, 2, 2, 2], last_layer_dim=2048, fc_hidden_dim=hidden_dim)
...
class BinaryHyperParamsResearch:
    ...
    def __init__(self, model_cls, config_file_path, config: ConfigParser, dataset_name, candidates_network_archs, train_batch_dim=128,
                 test_batch_dim=64):
        self.models = _generate_model(model_cls[0], model_cls[1],  candidates_network_archs)
        ...
        self.save_folder_reduced_candidates = os.path.join(RESULTS_FOLDER, dataset_name, "reduced_models")
    ...
    def binary_search(self, limit_min_increment, limit_max_increment, steps_limit, trainer_manager, init_model_path):
        ...
        # Creazione modello con solo layer fc1 e fc2
        fc1_w, fc1_b = baseline_model_ref.fc1.weight.data, baseline_model_ref.fc1.bias.data
        fc2_w, fc2_b = baseline_model_ref.fc2.weight.data, baseline_model_ref.fc2.bias.data
        reduced_base_model = FCHeadOnly(fc1_w, fc1_b, fc2_w, fc2_b).to(device)

        dummy_input_fc = torch.randn(
            1,
            reduced_base_model.fc1.in_features,
            device=self.device
        )
        # Save results
        save_models(reduced_base_model, reduced_base_model.identifier, self.save_folder_reduced_candidates, self.device,
                    dummy_input_fc)
        ...
        for idx, model_untr in enumerate(self.models):
            ...
            if not failure_bool:
                ...
                # Creazione modello con solo layer fc1 e fc2
                fc1_w, fc1_b = model_ref.fc1.weight.data, model_ref.fc1.bias.data
                fc2_w, fc2_b = model_ref.fc2.weight.data, model_ref.fc2.bias.data
                reduced_model = FCHeadOnly(fc1_w, fc1_b, fc2_w, fc2_b).to(self.device)

                # Dummy input corretto per FCHeadOnly
                dummy_input_fc = torch.randn(
                    1,
                    model_ref.fc1.in_features,
                    device=self.device
                )

                # Save results
                save_models(reduced_model, reduced_model.identifier, self.save_folder_reduced_candidates, self.device,
                            dummy_input_fc)
